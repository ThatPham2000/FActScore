...
class AtomicFactGenerator(object):
    def __init__(self, key_path, demon_dir, cache_file=None):
        ...
        self.is_bio = True  # always true for original FActScore research
        ...
        # List<List<String>>: document tokens
    ...
    def get_atomic_facts_from_paragraph(self, paragraphs, cost_estimate=None):
        """Get the atomic facts from the paragraphs. Return a total words cost if cost_estimate != None.
        Input: [paragraph]
        Output: (sent, [atomic_fact]) , para_breaks"""
        sentences = []
        para_breaks = []
        ...
        for i, sent in enumerate(sentences):
            condition1 = not self.is_bio
            condition2 = i == 0 and (sent.startswith("Sure") or sent.startswith("Here are"))
            condition3 = (i == len(sentences) - 1 and (
        ...
        if cost_estimate:
            ...
            # Map<String, List<String>> => {sent: [atomic_fact]}
    ...
    def get_init_atomic_facts_from_sentence(self, sentences, cost_estimate=None):
        ...
        for sentence in sentences:
            if sentence in atoms:
                continue
            top_k_matching = best_demos(sentence, self.bm25, list(demons.keys()), k)
            prompt = "Given the following examples of prompting and generating atomic facts from a sentence. Atomic facts generated by LLM should start with the - character.\n\n"
            prompt = prompt + "N-shot examples:\n"

            for i in range(n):
                prompt = prompt + "Please breakdown the following sentence into independent facts with the same generation format and without any explanation: {}\n".format(
                    list(demons.keys())[i])
                for fact in demons[list(demons.keys())[i]]:
                    prompt = prompt + "- {}\n".format(fact)
                prompt = prompt + "\n"

            for match in top_k_matching:
                prompt = prompt + "Please breakdown the following sentence into independent facts with the same generation format and without any explanation: {}\n".format(
                    match)
...
def best_demos(query, bm25, demons_sents, k):
    ...
    top_k_matching = bm25.get_top_n(tokenized_query, demons_sents, k)
...
def extract_numeric_values(text):
    ...
    pattern = r'\b\d+(?:\.\d+)?(?:,\d{3})*\b'  # regular expression pattern for numbers with optional decimal and comma
...
def detect_initials(text):
    """
    Detect initials in the text. An initial is defined as a capital letter followed by a period and an optional space
    and another capital letter followed by a period. For example, "J. K." or "A. B.".
    """
    pattern = r"[A-Z]\. ?[A-Z]\."
...
def main(demon_dir: str):
    ...
    statement2 = 'Thierry Henry (born 17 August 1977) is a French professional football coach, pundit, and former player. He is considered one of the greatest strikers of all time, and one the greatest players of the Premier League history. He has been named Arsenal F.C\'s greatest ever player.\n\nHenry made his professional debut with Monaco in 1994 before signing for defending Serie A champions Juventus. However, limited playing time, coupled with disagreements with the club\'s hierarchy, led to him signing for Premier League club Arsenal for £11 million in 1999.\n\nJ. K. Rowling is a famous author. A. B. Smith is a fictional name.'
    atomic_facts, para_breaks = generator.run(statement2)
